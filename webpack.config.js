...
const assetsPath              = path.resolve(__dirname,"./src/client/static");
const buildPath               = path.resolve(__dirname, "./src/client/build");
const nodemodulesPath = path.resolve(__dirname, './node_modules/');
const includePath           = path.resolve(__dirname, './src/client/');
const port                        = 3001;  //devServer端口号
const publicPath = '/';
...
const config = {
    ...
    devServer: {
        host: '0.0.0.0',
        compress: false, //true为开启gzip
        //quiet: true, //安静模式。不输出任何日志到命令行界面
        clientLogLevel: 'none',
    ...
    module: {
        loaders: [{
                ...
                */
                ...
                //======使用happypack后的配置=========
                //注意 loaders: ['happypack/loader?id=css']中的id;

                test: /\.(css|scss)$/, //查找带.css后缀名的文件进行打包，这里test是一个正则;
                loaders: ['happypack/loader?id=css'], //这里是处理匹配文件的方法，如果不用这个配置文件的时候是style!和css!，注意执行顺序是从右到左，所以把css放第二个参数;
    ...
    plugins: [
        ...
        //     id: 'assets',
        //     threadPool: happyThreadPool,
        //     loaders: ['url-loader!file-loader?limit=8192&name=images/[name].[ext]'],
        //     cache: true,
        //     verbose: true
        // }),
        ...
        new HappyPack({
            id: 'assets',
            threadPool: happyThreadPool,
            loaders: ['file-loader?limit=8192&name=images/[name].[ext]'],
            cache: true,
            verbose: true
        }),
